...
for device_adb, device, port, serial in zip(devices_adb, devices, ports, serials):
    ...
    device_adb.shell("su -c 'cd /data/data/com.termux/files/home/wmn_research && chmod +x ./socket/quic_tcpdump.py'")
    client_tcpdump_cmd = f"cd /data/data/com.termux/files/home/wmn_research && python3 ./socket/quic_tcpdump.py -d {device} -p {portString}"
    adb_tcpdump_cmd = f"su -c '{client_tcpdump_cmd}'"
    
    device_adb.shell("su -c 'cd /data/data/com.termux/files/home/wmn_research && chmod +x ./client_phone/client_socket.sh'")
    su_cmd = f'cd /data/data/com.termux/files/home/wmn_research && ./client_phone/client_socket.sh {device} {portString} {total_time} {bitrate} {length}'
